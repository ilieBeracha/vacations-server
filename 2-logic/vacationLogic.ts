...
export async function getAllVacations(userId: number, offset: number, likes: string, active: string, coming: string) {
    ...
    const query = `SELECT id ,destination, description, DATE_FORMAT(startingDate, "%Y-%m-%d") AS startingDate, DATE_FORMAT(endingDate, "%Y-%m-%d") AS endingDate, price, imageName, (select vl.userId from likes vl where vl.userId = ${userId} and vl.vacationId = v.id) as userLikes, (select count(*) from likes vl where vl.vacationId = v.id) as totalLikes from vacation v ORDER BY startingDate limit 10 offset ${offset}`
    const [results] = await execute<OkPacket>(query);
    return results;
}
...
export async function addVacation(vacation: VacationInterface, file: any) {
    ...
    const query = 'INSERT INTO vacation(destination,description,startingDate,endingDate,price,imageName) VALUES(?,?,?,?,?,?)';
...
export async function editVacation(vacation: VacationInterface, file: any) {
    if (file) {
        ...
        const getPrevFileFromDb = 'SELECT imageName FROM vacation WHERE id = ?'
        ...
        const query = 'UPDATE vacation SET destination = ?, description = ?, startingDate = ?, endingDate = ?, price = ?,imageName = ? WHERE id = ?';
        const [results] = await execute<OkPacket>(query, [destination, description, startingDate, endingDate, price, key, id]);
        return results;
    } else {
        const { destination, description, startingDate, endingDate, price, id } = vacation;
        const query = 'UPDATE vacation SET destination = ?, description = ?, startingDate = ?, endingDate = ?, price = ? WHERE id = ?';
...
export async function getSumOfVacations(userId:number,likes: any, active: any, coming: any) {
    if (likes === 'true') {
        const query = `SELECT count(*) as vacationsSum FROM vacation JOIN likes ON vacation.Id = likes.vacationId WHERE likes.userId = ${userId}`
        const [results] = await execute(query);
        return results;
    } else if (active === "true") {
        const query = 'SELECT count(*) as vacationsSum FROM vacation WHERE startingDate < now() AND endingDate > now()'
        const [results] = await execute(query);
        return results;
    } else if (coming === "true") {
        const query = 'SELECT count(*) as vacationsSum FROM vacation where startingDate > now()'
        const [results] = await execute(query);
        return results;
    }
    const query = 'SELECT count(*) as vacationsSum FROM vacation';
    const [results] = await execute(query);
    return results;
}